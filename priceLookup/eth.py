...
class ETHPriceLookup(BasePriceLookup):
    @property
    def unit(self) -> str:
        return "ETH"
    ...
    def load(self) -> None:
        ...
        with open(self.source_file_path, "r") as f:
            ...
            self.price_data = list(map(self._csv_row_to_eth_price_dto, reader))
            self.price_data.sort(key=lambda x: x.timestamp, reverse=False)

    @staticmethod
    def _csv_row_to_eth_price_dto(row: Dict[str, Any]) -> PriceDto:
